...
class EmptyMap(MapFactory):
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            ...
            coord = (random.randint(1, 14), random.randint(1, 10))
            intersect = True
            while intersect:
                intersect = False
                if _map[coord[1]][coord[0]] == wall:
    ...
    class Map:
        def __init__(self):
            self.Map = ['00000000000000000000000000000000',
                        ...
                        '0  0      0   0  0      0      0',
                        '0  00000  00000  00000  0      0',
                        '0      0  0      0      0      0',
                        '0  00000  0      00000  00000  0',
                        '0                              0',
                        '0                              0',
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            ...
            for obj_name in object_list_prob['objects']:
                prop = object_list_prob['objects'][obj_name]
                for i in range(random.randint(prop['min-count'], prop['max-count'])):
                    coord = (random.randint(1, 31), random.randint(1, 10))
                    intersect = True
                    while intersect:
                        intersect = False
                        if _map[coord[1]][coord[0]] == wall:
                            intersect = True
                            coord = (random.randint(1, 31),
                                     random.randint(1, 10))
                            continue
                        for obj in self.objects:
                            if coord == obj.position or coord == (1, 1):
            ...
            for obj_name in object_list_prob['enemies']:
                prop = object_list_prob['enemies'][obj_name]
                for i in range(random.randint(0, 5)):
                    coord = (random.randint(1, 23), random.randint(1, 10))
                    intersect = True
                    while intersect:
                        intersect = False
                        if _map[coord[1]][coord[0]] == wall:
                            intersect = True
                            coord = (random.randint(1, 23),
                                     random.randint(1, 10))
                            continue
                        for obj in self.objects:
                            if coord == obj.position or coord == (1, 1):
